...
if (isset($_GET['match_no']) && !empty($_GET['match_no'])) {
    ...
    $sql_fetch_match = "SELECT home_team, away_team, winning_team, nrr FROM tournament_data WHERE match_no = '$match_no'";
    ...
    if ($result_fetch_match->num_rows > 0) {
        ...
        if ($result_fetch_winning_team_short_name->num_rows > 0) {
            ...
            $winning_team = $winning_team_row['short_name'];
        }
        ...
        if ($conn->query($sql_delete_match) === TRUE) {
            if ($winning_team !== NULL && $losing_team !== NULL) {
                // Update wins and points for the winning_team
                $update_winning_team = "UPDATE teams SET wins = wins - 1, points = points - 2, matches_played = matches_played - 1, nrr = nrr - $nrr WHERE short_name = '$winning_team'";
                if ($conn->query($update_winning_team) !== TRUE) {
                    echo "Error updating wins and points for winning team: " . $conn->error;
                }

                // Update losses for the losing_team
                ...
                $update_tied_team_one = "UPDATE teams SET no_result = no_result - 1, points = points - 1, matches_played = matches_played - 1, nrr = nrr - $nrr WHERE short_name = '$home_team'";
                if ($conn->query($update_tied_team_one) !== TRUE) {
                    echo "Error updating matches played and points for tied team 1: " . $conn->error;
                }

                $update_tied_team_two = "UPDATE teams SET no_result = no_result - 1, points = points - 1, matches_played = matches_played - 1, nrr = nrr - $nrr WHERE short_name = '$away_team'";
