...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (
        ...
        isset($_POST['inning2_overs']) && isset($_POST['inning2_balls']) && isset($_POST['reduced_match_overs'])
    ) {
        // Get form data
        ...
        $reduced_match_overs = intval($_POST['reduced_match_overs']);

        // Calculate overs and balls for NRR based on wickets lost
        // Inning-1: maximum of $reduced_match_overs set to first inning overs
        $inning1_overs_nrr = $reduced_match_overs;
        $inning1_balls_nrr = 0;

        // Inning-2
        if ($inning2_runs == 0) {
            $inning2_overs_nrr = $reduced_match_overs;
            $inning2_balls_nrr = 0;
        } else {
            ...
            if ($inning2_overs == 0 && $inning2_balls == 0 && $inning2_runs > 0) {
                ...
                $inning2_wickets == 10 || (
                    // Team retired early (not all out, but failed chase) — assume full overs for NRR
                    $inning2_runs < $inning1_runs && $inning2_wickets != 10)
            ) {
        ...
        if ($result_set->num_rows > 0) {
            ...
            if ($inning1_runs > $inning2_runs) {
                $resultMessage = "$batting_team won by " . ($inning1_runs - $inning2_runs) . " runs. ($reduced_match_overs overs game - due to rain.)";
                $winning_team = $batting_team;
                $losing_team = $bowling_team;
            } elseif ($inning1_runs < $inning2_runs) {
                $resultMessage = "$bowling_team won by " . (10 - $inning2_wickets) . " wickets. ($reduced_match_overs overs game - due to rain.)";
                ...
                if (!empty($_POST['super_over_winner'])) {
                    ...
                    $resultMessage = "Match tied. $super_over_winner won the match in Super Over. ($reduced_match_overs overs game - due to rain.)";
                }
            }
